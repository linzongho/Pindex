...
namespace Pindex\Core\Storage;
use Pindex\Core\Storage;
use Pindex\Core\StorageInterface;
...
//----------------------------------------------------------------------------------------------------------------------
    ...
    public function touch($filepath, $mtime = null, $atime = null){
        ...
        $this->checkAndMakeSubdir($filepath) or PindexException::throwing("Check path '$filepath' failed");
    ...
    private function rmdir($dir, $recursion=false){
        ...
        while ($file = readdir($dh)) {
            ...
            $dir = PINDEX_IS_WIN?str_replace('\\','/',"{$dir}/{$file}"):"{$dir}/{$file}";
