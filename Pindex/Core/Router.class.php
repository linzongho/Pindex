...
//            \Pindex\dumpout($modules);
    ...
    public static function createInAPI($modules,$contler,$action,array $params=null){
        $_config = self::getConfig();
        is_array($modules) and $modules = self::toModulesString($modules,$_config['MM_BRIDGE']);
    ...
    public static function createHostname($modules,&$flag){
        $_config = self::getConfig();
        //模块标识符
        $mid = is_array($modules)?self::toModulesString($modules,$_config['MM_BRIDGE']):$modules;
