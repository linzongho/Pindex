...
//                    \Pindex\dumpout($templateFile,$tmplCacheFile,Storage::mtime($templateFile),Storage::mtime($tmplCacheFile));
    ...
    protected function parsePhp($content) {
        ...
        if($this->config['DENY_PHP'] && false !== strpos($content,'<?php')) {
            PindexException::throwing('denyed using raw php in template!');
        }
        return $content;
    }
...
//        \Pindex\dumpout($begin,$end,$matches);
    ...
    private function parseXmlAttrs($attrs) {
        ...
        if(!$xml)  return PindexException::throwing('_XML_TAG_ERROR_:'.$attrs);
...
//            \Pindex\dump($templateName,dirname($this->template).'/'.$templateName);
            if(strpos($templateName,'/') !== 0){
                ...
                //Absolute path
                $templateName = PINDEX_PATH_APP.$templateName;//abs
            }
            if(is_file($templateName)){
                // 获取模板文件内容
                $parseStr .= file_get_contents($templateName);
            }else{
                PindexException::throwing($templateName,'not found!');
            }
        }
        return $parseStr;
    }
    ...
    public function parseXmlAttr($attr,$tag) {
        ...
        $xml    =   simplexml_load_string($xml);
        $xml    or  PindexException::throwing('_XML_TAG_ERROR_:'.$attr);
        $xml    =   (array)($xml->tag->attributes());
            ...
            if($array) {
                ...
                foreach($attrs as $name) {
                    if( isset($array[$name])) {
                        ...
                        return PindexException::throwing('_PARAM_ERROR_:'.$name);
                    }
                }
                return $array;
            }
...
//            $basepath   =   !empty($tag['basepath'])?$tag['basepath']:PINDEX_PATH_RUNTIME.'/Public';
            $basepath   =   !empty($tag['basepath'])?$tag['basepath']:PINDEX_PATH_RUNTIME.'/Public';
            // 命名空间方式导入外部文件
            $array      =   explode(',',$file);
            foreach ($array as $val){
    ...
    // import别名 采用文件方式加载(要使用命名空间必须用import) 例如 <load file="PINDEX_PATH_RUNTIME/Js/Base.js" />
    public function _load($tag) {
        return $this->_import($tag,true);
    }

    // import别名使用 导入css文件 <css file="PINDEX_PATH_RUNTIME/Css/Base.css" />
    public function _css($tag) {
        return $this->_import($tag,true,'css');
    }

    // import别名使用 导入js文件 <js file="PINDEX_PATH_RUNTIME/Js/Base.js" />
