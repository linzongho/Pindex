...
//        $xml   .= self::data_to_xml($data, $item, $id);
	 ...
	 * @return boolean
	 */
	abstract protected function setCache($cachename,$value,$expired);
	 ...
	 * @return mixed
	 */
    abstract protected function getCache($cachename);
	 ...
	 * @return boolean
	 */
    abstract protected function removeCache($cachename);
	...
	/**
	 * 获取微信服务器IP地址列表
	 * @return false|array('127.0.0.1','127.0.0.1')
	 */
	...
	/**
	 * 获取菜单(认证后的订阅号可用)
	 * @return false|array('menu'=>array(....s))
     */
     ...
     * @param string $media_id 媒体文件id
     * @ param boolean $is_video 是否为视频文件，默认为false
     * @return boolean|array data
     */
    public function getForeverMedia($media_id){
        if (!$this->access_token && !$this->checkAuth()) return false;
	 ...
	 * @return false|array('ticket'=>'qrcode字串','expire_seconds'=>604800,'url'=>'二维码图片解析后的地址')
	 */
	 ...
	 * @return false|array {subscribe,openid,nickname,sex,city,province,country,language,headimgurl,subscribe_time,[unionid]}
	 ...
	 * @return false|array user_info_list{subscribe,openid,nickname,sex,city,province,country,language,headimgurl,subscribe_time,[unionid]}{}{}...
	 * 注意：unionid字段 只有在用户将公众号绑定到微信开放平台账号后，才会出现。建议调用前用isset()检测一下
	 */
	 ...
	 * @return false|array {access_token,expires_in,refresh_token,openid,scope}
	 */
	 ...
	 * @param string $openid
	 * @return false|array {openid,nickname,sex,province,city,country,headimgurl,privilege,[unionid]}
	 * 注意：unionid字段 只有在用户将公众号绑定到微信开放平台账号后，才会出现。建议调用前用isset()检测一下
	 */
	...
	public function setTMIndustry($id1,$id2=''){
	    ...
	    if (!$this->access_token && !$this->checkAuth()) return false;
	    $result = $this->http_post(self::API_URL_PREFIX.self::TEMPLATE_SET_INDUSTRY_URL.'access_token='.
	 ...
	 * @return boolean|array
	 */
	public function querySemantic($query,$category,$latitude=0,$longitude=0,$city="",$region=""){
	    if (!$this->access_token && !$this->checkAuth()) return false;
	    $data=array(
	            'query' => $query,
	            'category' => $category,
