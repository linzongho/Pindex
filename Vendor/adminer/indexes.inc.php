...
<th><noscript><input type='image' class='icon' name='add[0]' src='static/adminer/plus.gif' alt='+' title='<?php echo lang('Add next'); ?>'></noscript>&nbsp;
</thead>
<?php
if ($primary) {
...
foreach ($row["indexes"] as $index) {
	if (!$_POST["drop_col"] || $j != key($_POST["drop_col"])) {
		...
		echo "<td><input type='image' class='icon' name='drop_col[$j]' src='static/adminer/cross.gif' alt='x' title='" . lang('Remove') . "' onclick=\"return !editingRemoveRow(this, 'indexes\$1[type]');\">\n";
	}
	$j++;
}
?>
