    ...
    <div class="form" style="padding: 10px 20px;">
        ...
        if ($error == '') {
            echo '<div class="success"><h4>Successful!</h4>', $L['install_login'],
                '<br/>' . $L['install_user_default'] . '</div></div><div class="guest"><a href="./' . $entry_name . '?user/loginFirst">' . $L['login'] . '</a></div>';
        } else {
            echo '<div class="error"><h4>error:</h4>' . $error . '</div></div><div class="guest"><a href="./{$entry_name}?user/loginFirst">' . $L['php_env_error_ignore'] . '</a></div>';
        }
        ?>
    </div>
</div>
